...
export const addFunds = async (req: Request, res: Response) => {
	...
	// Inject BFLA flag
	okResponse(res, { isSuccess: true, message: 'flag{$InfiniteMoneyGlitch$}' });
};
...
export const sellPosition = async (req: Request, res: Response) => {
	...
	// BOLA Check: Is the user trying to sell someone else's stock?
	let bolaFlag = null;
	if (req.user && req.user.id !== userId) {
		bolaFlag = 'flag{DimondHands2023}';
	}
	...
	const responsePayload: any = { isSuccess: true };
	if (bolaFlag) {
		responsePayload.message = bolaFlag;
	}
...
export const getPortfolio = async (req: Request, res: Response) => {
	try {
		...
		if (req.user && req.user.isAdmin) {
			responsePayload.message = 'flag{B0l@B0l@$$$$}';
		}
